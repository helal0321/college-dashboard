    ...
    <div className="bg-main h-[100vh] overflow-y-scroll">
      ...
      >
        <h1 className="text-center text-4xl font-bold text-third">
          {language === "english"
            ? "add or view or delete quality"
            : "إضافة أو عرض أو حذف الجودة"}
        </h1>
        <p className="font-bold text-center text-xl mt-4">
          {language === "english"
            ? "here you can manage quailty data"
            : "هنا يمكنك إدارة بيانات الجودة"}
        </p>
        <p className="mt-10 text-2xl font-bold text-center">
          {language === "english" ? "add new quality:" : "اضافة بيانات جودة"}
        </p>
        <Link
          href="add-quality"
          className="bg-third w-fit mx-auto text-lg px-8 py-4 rounded-lg block mt-6"
        >
          {language === "english" ? "add quality:" : "اضافة جودة"}
        </Link>
        <p className="text-center font-bold text-4xl my-10">
          {language === "english" ? "list of all quality" : "قائمة الجودة"}
        </p>
        {loading ? (
            ...
            {quality.map((singleQuality, index) => (
              ...
              >
                ...
                <div className="w-1/6 flex justify-center relative">
                  ...
                  >
                    <Link
                      href={`/update-quality/${singleQuality.id}`}
                    ...
                    <button
                      ...
                      onClick={() => {
                        fetch(`${baseUrl}/Delete_Quality/${singleQuality.id}`, {
                          method: "delete",
                          headers: {
                            "Content-Type": "application/json",
                            Authorization: `Bearer ${
                          ...
                          .then((res) => {
                            ...
                            setQuality(
                              quality.filter(
                                (element) => singleQuality.id !== element.id
                              )
                            );
