  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const res = await fetch(
        `${baseUrl}/api/Department/Get_All_Departments_Details/${language === "english" ? "eng" : "ar"}`,
        {
          method: "GET",
    ...
    <div className="bg-main h-[100vh] overflow-y-scroll">
      ...
      >
        <h1 className="text-center text-4xl font-bold text-third">
          {language === "english"
            ? "add or view or delete Department"
            : "إضافة أو عرض أو حذف الأقسام"}
        </h1>
        <p className="font-bold text-center text-xl mt-4 ">
          {language === "english"
            ? "here you can manage Department Data"
            : "هنا يمكنك ادارة بيانات الاقسام"}
        </p>
        <p className="mt-10 text-2xl font-bold text-center">
          {language === "english" ? "add New Department:" : "أضافه قسم جديد"}
        </p>
        <Link
          href="add-department"
          className="bg-third w-fit mx-auto text-lg px-8 py-4 rounded-lg block mt-6"
        >
          {language === "english" ? "add Department" : "اضافة قسم"}
        </Link>
        ...
        {loading ? (
          ...
          <div className="bg-secondary p-6 rounded-lg">
            <div className="flex justify-between items-center border border-x-0 border-t-0 pb-2 border-third">
              <p className="text-center w-1/4 font-bold text-third">
            ...
            {departments.map((department, index) => (
              ...
              >
                <p className="text-center w-1/4">{department.name}</p>
                <p className="text-center w-1/4">{`${department.description.slice(0, 120)}...`}</p>
                <p className="text-center w-1/4">{department.empName}</p>
                ...
                <div className="w-1/6 flex justify-center relative">
                  ...
                  >
                    <Link
                      href={`/add-employee-D/${department.id}`}
                      className="bg-orange-600 text-sm mb-2 px-2 py-1 rounded-md text-center"
                    >
                      {language === "english" ? "add employee" : "اضافة موظف"}
                    </Link>
                    <Link
                      href={`/delete-employee-D/${department.id}`}
                      className="bg-red-600 text-sm mb-2 px-2 py-1 rounded-md text-center"
                    >
                      {language === "english" ? "delete employee" : "حذف موظف"}
                    </Link>
                    <Link
                      href={`/update-department/${department.id}`}
                    ...
                    <button
                      ...
                      onClick={() => {
                        fetch(`${baseUrl}/Delete_Department/${department.id}`, {
                          method: "delete",
                          headers: {
                            "Content-Type": "application/json",
                            Authorization: `Bearer ${
                          ...
                          .then((res) => {
                            ...
                            setDepartments(
                              departments.filter(
                                (element) => department.id !== element.id
                              )
                            );
