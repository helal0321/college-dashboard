    ...
    <div className="bg-main h-[100vh] overflow-y-scroll">
      ...
      >
        <h1 className="text-center text-4xl font-bold text-third">
          {language === "english"
            ? "add or view or delete Units"
            : "إضافة أو عرض أو حذف الوحدات"}
        </h1>
        <p className="font-bold text-center text-xl mt-4">
          {language === "english"
            ? "here you can manage Units Data"
            : "هنا يمكنك إدارة بيانات الوحدات"}
        </p>
        <p className="mt-10 text-2xl font-bold text-center">
          {language === "english" ? "add New Unit:" : "اضافة وحدة جديدة"}
        </p>
        <Link
          href="add-unit"
          className="bg-third w-fit mx-auto text-lg px-8 py-4 rounded-lg block mt-6"
        >
          {language === "english" ? "add Unit:" : "اضافة وحدة"}
        </Link>
        <p className="text-center font-bold text-4xl my-10">
          {language === "english" ? "list of all Units" : "قائمة بجميع الوحدات"}
        </p>
        {loading ? (
          ...
          <div className="bg-secondary p-6 rounded-lg">
            ...
            {units.map((unit, index) => (
              ...
              >
                <p className="text-center w-1/6">{unit.name}</p>
                <p className="text-center w-1/6">{unit.description}</p>
                ...
                <div className="w-1/6 flex justify-center relative">
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => {
                        fetch(`${baseUrl}/api/Unit/${unit.id}`, {
                          method: "delete",
                          headers: {
                            "Content-Type": "application/json",
                            Authorization: `Bearer ${
                          ...
                          .then((res) => {
                            console.log(res, "hjg");
                            setUnits(
                              units.filter((element) => unit.id !== element.id)
                            );
