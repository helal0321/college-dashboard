    ...
    <div className="bg-main h-[100vh] overflow-y-scroll">
      ...
      >
        <h1 className="text-center text-4xl font-bold text-third">
          {language === "english"
            ? "add or view or delete news"
            : "إضافة أو عرض أو حذف الأخبار"}
        </h1>
        <p className="font-bold text-center text-xl mt-4">
          {language === "english"
            ? "here you can manage news data"
            : "هنا يمكنك إدارة بيانات الأخبار"}
        </p>
        <p className="mt-10 text-2xl font-bold text-center">
          {language === "english" ? "add news:" : "اضافة خبر جديد"}
        </p>
        <Link
          href="add-news"
        ...
        <p className="text-center font-bold text-4xl my-10">
          {language === "english" ? "list of all news" : "قائمة بجميع الأخبار"}
        </p>
        {loading ? (
            ...
            {news.map((New, index) => (
              ...
              >
                <p className="text-center w-1/6">
                  <img src={New.img} className="w-20 h-20" />
                </p>
                <p className="text-center w-1/6">{New.name}</p>
                <p className="text-center w-1/6">{New.news_Date}</p>
                ...
                <div className="w-1/6 flex justify-center relative">
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => {
                        fetch(`${baseUrl}/Delete_News/${New.newsId}`, {
                          method: "delete",
                          headers: {
                            "Content-Type": "application/json",
                            Authorization: `Bearer ${
                          ...
                          .then((res) => {
                            ...
                            setNews(
                              news.filter(
                                (element) => New.newsId !== element.newsId
                              )
                            );
