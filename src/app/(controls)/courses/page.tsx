  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const res = await fetch(
        `${baseUrl}/api/Course/Get_All_Courses/${language === "english" ? "eng" : "ar"}`,
        {
          method: "GET",
        ...
        {loading ? (
          ...
          <div className="bg-secondary p-6 rounded-lg">
            <div className="flex justify-between items-center border border-x-0 border-t-0 pb-2 border-third">
              ...
              <p className="text-center w-1/6 font-bold text-third">
                {language === "english" ? "level year" : "السنة الدراسية"}
              </p>
              <p className="text-center w-1/6 font-bold text-third">
            ...
            {courses.map((course, index) => (
              ...
              >
                ...
                <p className="text-center w-1/6">{course.levelYear}</p>
                <p className="text-center w-1/6">
                  <a href={course.pdfDescription} target="_blank">
                    pdf file
                  </a>
                </p>
                <p className="text-center w-1/6">{course.departmentName}</p>
                ...
                <div className="w-1/6 flex justify-center relative">
                  ...
                  >
                    <Link
                      href={`/update-courses/${course.courseId}`}
                    ...
                    <button
                      ...
                      onClick={() => {
                        fetch(`${baseUrl}/Delete_Course/${course.courseId}`, {
                          method: "delete",
                          headers: {
                            "Content-Type": "application/json",
                            Authorization: `Bearer ${
                          ...
                          .then((res) => {
                            ...
                            setCourses(
                              courses.filter(
                                (element) =>
                                  course.courseId !== element.courseId
                              )
                            );
